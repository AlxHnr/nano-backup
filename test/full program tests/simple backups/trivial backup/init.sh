...
expected_files=$(grep -E '^Total: \+[0-9]+ items' expected-output | \
  sed -r 's,^Total: \+([0-9]+) items.*$,\1,g')
total_files=$(($expected_files + $test_data_path_elements))

mkdir -p generated/repo

sed -r "s,^\/,${path_to_test_data}/,g" config > generated/repo/config

sed -r "s,^(.. )\/(.*)$,\1${path_to_test_data}/\2,g" expected-output | \
