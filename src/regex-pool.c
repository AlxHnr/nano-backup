...
*/
const regex_t *rpCompile(String expression,
                         String file_name,
                         size_t line_nr)
{
  regex_t *regex = CR_RegionAlloc(CR_GetGlobalRegion(), sizeof(*regex));
  const int error = regcomp(regex, expression.content,
  ...
  if(error != 0)
  {
    ...
    die("%s: line %zu: %s: \"%s\"", file_name.content,
        line_nr, error_str, expression.content);
  }
